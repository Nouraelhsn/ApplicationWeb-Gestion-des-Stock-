...
function getArticle($id=null, $searchDATA = array())
{
         if (!empty($id)) {
          ...
          return $req->fetch();
            ...
            if(!empty($date_fabrication)) $search .= " AND a.date_fabrication = $date_fabrication ";
            if(!empty($date_expiration)) $search .= " AND a.date_expiration = $date_expiration ";
            ...
            $sql = "SELECT nom_article, libelle_categorie, quantite, prix_unitaire, date_fabrication, 
           date_expiration, id_categorie, a.id  FROM article AS a, categorie_article AS c WHERE c.id=a.id_categorie $search";
